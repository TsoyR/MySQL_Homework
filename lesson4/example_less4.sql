DROP DATABASE IF EXISTS example_lesson4;
CREATE DATABASE example_lesson4;
USE example_lesson4;
...
SELECT media.filename,users.id,users.email
FROM media,users
WHERE users.id IN (SELECT id FROM users WHERE email = "arlo50@example.org");
