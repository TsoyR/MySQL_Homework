...
SELECT s.*,c.*
FROM shops s
JOIN (SELECT id, name, shops_id FROM cats WHERE name = "Murzik") c
ON s.id = c.shops_id;
...
ord_an — ID анализа.
*/
SELECT an.an_name,an.an_price,o.ord_datetime
FROM Analysis AS an
JOIN(SELECT ord_id,ord_datetime,ord_an FROM Orders WHERE ord_datetime BETWEEN '2020-02-05' AND '2020-02-13')o
